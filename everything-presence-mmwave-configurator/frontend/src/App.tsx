...
function App() {
	...
	const [newRoomDeviceId, setNewRoomDeviceId] = useState<string | undefined>(
		undefined,
	);
	...
	const [wizardPlacementDone, setWizardPlacementDone] =
	...
	useEffect(() => {
		const load = async () => {
			try {
				setLoading(true);
				const [deviceRes, profileRes, roomsRes, settingsRes] =
					await Promise.all([
						fetchDevices(),
						fetchProfiles(),
						fetchRooms(),
				...
				if (
					!settingsRes.settings.wizardCompleted &&
					roomsRes.rooms.length === 0
				) {
					setView("wizard");
				} else if (
					!settingsRes.settings.wizardCompleted &&
						...
						if (message.type === "subscribed") {
							...
							setLiveState((prev) => {
								...
								if (message.entityId.match(/zone_(\d+)_occupancy/)) {
									...
									message.entityId.includes("target_distance") ||
									message.entityId.includes("mmwave_target_distance")
								) {
									updated.distance = parseFloat(message.state) || null;
								} else if (
									message.entityId.includes("target_speed") ||
									message.entityId.includes("mmwave_target_speed")
								) {
									updated.speed = parseFloat(message.state) || null;
								} else if (
									message.entityId.includes("target_energy") ||
									message.entityId.includes("mmwave_target_energy")
								) {
									...
									message.entityId.includes("_mmwave") &&
									!message.entityId.includes("_target") &&
									!message.entityId.includes("_mode") &&
									!message.entityId.includes("_distance") &&
									!message.entityId.includes("_sensitivity") &&
									!message.entityId.includes("_latency") &&
									!message.entityId.includes("_threshold")
								) {
								...
								// Handle EP1 config entities
									...
									if (!updated.config) updated.config = {} as any;
									updated.config.distanceMin =
										parseFloat(message.state) || null;
								} else if (message.entityId.includes("max_distance")) {
									if (!updated.config) updated.config = {} as any;
									updated.config.distanceMax =
										parseFloat(message.state) || null;
								} else if (message.entityId.includes("trigger_distance")) {
									if (!updated.config) updated.config = {} as any;
									updated.config.triggerDistance =
										parseFloat(message.state) || null;
								}
								// Handle EPL max distance entity (number.${name}_distance)
								else if (
									message.entityId.match(/number\.\w+_distance$/) &&
									!message.entityId.includes("target") &&
									!message.entityId.includes("trigger")
								) {
									if (!updated.config) updated.config = {} as any;
									updated.config.distanceMax =
										parseFloat(message.state) || null;
								...
								// Handle EPL target tracking entities
									...
									message.entityId.match(
										/target_(\d+)_(x|y|distance|speed|angle|resolution)/,
									)
								) {
									// Parse target position entities (e.g., sensor.ep1_target_1_x)
									const match = message.entityId.match(
										/target_(\d+)_(x|y|distance|speed|angle|resolution)/,
									);
									if (match) {
								...
								// Handle assumed presence status (entry/exit feature)
									...
									message.entityId.includes("assumed_present_remaining")
								) {
									const value = parseFloat(message.state);
									updated.assumedPresentRemaining = !isNaN(value)
										? value
										: undefined;
	...
	const deviceToRoom = React.useCallback(
	...
	const handleCreateRoom = async () => {
		...
		try {
			...
			const selectedDevice = newRoomDeviceId
				? devices.find((d) => d.id === newRoomDeviceId)
				: undefined;
				...
				{view === "zoneEditor" && (
					<ZoneEditorPage
						onNavigate={(targetView) => {
							...
							const mappedView =
								targetView === "liveDashboard" ? "dashboard" : targetView;
							setView(mappedView);
						}}
						initialRoomId={selectedRoomId}
						initialProfileId={selectedProfileId}
						onWizardZonesReady={() => {
							...
							setWizardStep("finish");
							updateSettings({ zonesReady: true, wizardStep: "finish" }).catch(
								() => null,
							);
						}}
						liveState={liveState}
