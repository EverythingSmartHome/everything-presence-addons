...
if SUPERVISOR_TOKEN:
    ...
    log.error("No SUPERVISOR_TOKEN found and no HA_URL and HA_TOKEN provided.")
    sys.exit(1)


def check_connectivity() -> None:
    """Check connectivity with Home Assistant API."""
    try:
        response = requests.get(
            f"{HOME_ASSISTANT_API}/", headers=headers, timeout=10, verify=VERIFY_SSL
        )

        if response.status_code != HTTPStatus.OK:
            log.error(
                "Failed to connect to Home Assistant API. Status Code: %s",
                response.status_code,
            )
            log.error("Response: %s", response.text[:200])
    except requests.exceptions.RequestException:
        log.exception("Exception connecting to Home Assistant API")
...
def health_check() -> tuple[dict[str, Any], int]:
    ...
    try:
        log.error("🩺 Health check requested")
        response = requests.get(
            f"{HOME_ASSISTANT_API}/", headers=headers, timeout=5, verify=VERIFY_SSL
        )
...
def execute_template() -> tuple[dict[str, Any], int]:
    """Execute a Jinja2 template.

    forwarding it to Home Assistant's /api/template endpoint.
    It acts as a proxy, forwarding the template and returning the rendered result.
    """
    data = request.get_json()
    template = data.get("template")
    if not template:
        return jsonify({"error": "No template provided"}), HTTPStatus.BAD_REQUEST
    ...
    try:
        ...
        log.error(
            "Failed to execute template. Status Code: %s",
            response.status_code,
        )
        return jsonify(
            {
        ...
        log.exception("Connection error to HA API")
        return jsonify(
            {"error": "Cannot connect to Home Assistant API", "details": str(e)}
        ), HTTPStatus.BAD_GATEWAY
    except requests.exceptions.Timeout as e:
        log.exception("Timeout connecting to HA API")
        return jsonify(
            {"error": "Timeout connecting to Home Assistant API", "details": str(e)}
        ), HTTPStatus.GATEWAY_TIMEOUT
    except Exception as e:
        log.exception("Exception occurred while executing template")
        return jsonify(
            {"error": "Exception occurred while executing template", "details": str(e)}
        ), HTTPStatus.INTERNAL_SERVER_ERROR


@app.route("/api/entities/<entity_id>", methods=["GET"])
def get_entity_state(entity_id: str) -> tuple[dict[str, Any], int]:
    """Endpoint to get the state of a specific entity."""
    response = requests.get(
        f"{HOME_ASSISTANT_API}/states/{entity_id}",
        headers=headers,
        verify=VERIFY_SSL,
        timeout=10,
    )
...
def set_value() -> tuple[dict[str, Any], int]:
    """Set the value of a number entity via Home Assistant API."""
    try:
        data = request.json
        ...
        if not entity_id or value is None:
            return jsonify(
                {"error": "Missing entity_id or value"}
        ...
        response = requests.post(
            f"{HOME_ASSISTANT_API}/services/number/set_value",
            headers=headers,
            json=payload,
            verify=VERIFY_SSL,
            timeout=10,
        ...
        return jsonify(
            {
                "error": f"Failed to update entity {entity_id}.",
        ...
        return jsonify(
            {"error": "An error occurred while setting the value.", "details": str(e)}
        ), HTTPStatus.INTERNAL_SERVER_ERROR


@app.route("/api/services/switch/turn_on", methods=["POST"])
def switch_turn_on() -> tuple[dict[str, Any], int]:
    """Turn on a switch entity via Home Assistant API."""
    try:
        data = request.json
        entity_id = data.get("entity_id")

        if not entity_id:
            return jsonify({"error": "Missing entity_id"}), HTTPStatus.BAD_REQUEST

        payload = {"entity_id": entity_id}

        # Make the POST request to Home Assistant API
        response = requests.post(
            f"{HOME_ASSISTANT_API}/services/switch/turn_on",
            headers=headers,
            json=payload,
            verify=VERIFY_SSL,
            timeout=10,
        ...
        if response.status_code == HTTPStatus.OK:
            return jsonify(
                {"message": f"Switch {entity_id} turned on successfully."}
            ), HTTPStatus.OK
        return jsonify(
            {
                "error": f"Failed to turn on switch {entity_id}.",
                "details": response.text,
            }
        ), response.status_code
        ...
        return jsonify(
            {
                "error": "An error occurred while turning on the switch.",
                "details": str(e),
            }
        ), HTTPStatus.INTERNAL_SERVER_ERROR
...
def switch_turn_off() -> tuple[dict[str, Any], int]:
    """Turn off a switch entity via Home Assistant API."""
    try:
        data = request.json
        entity_id = data.get("entity_id")
        ...
        # Make the POST request to Home Assistant API
        response = requests.post(
            f"{HOME_ASSISTANT_API}/services/switch/turn_off",
            headers=headers,
            json=payload,
            verify=VERIFY_SSL,
            timeout=10,
        ...
        if response.status_code == HTTPStatus.OK:
            return jsonify(
                {"message": f"Switch {entity_id} turned off successfully."}
            ), HTTPStatus.OK
        return jsonify(
            {
                "error": f"Failed to turn off switch {entity_id}.",
                "details": response.text,
            }
        ), response.status_code
        ...
        return jsonify(
            {
                "error": "An error occurred while turning off the switch.",
                "details": str(e),
            }
        ), HTTPStatus.INTERNAL_SERVER_ERROR
...
def select_option() -> tuple[dict[str, Any], int]:
    """Select an option for a select entity via Home Assistant API."""
    try:
        data = request.json
        entity_id = data.get("entity_id")
        ...
        if not entity_id or option is None:
            return jsonify(
                {"error": "Missing entity_id or option"}
            ), HTTPStatus.BAD_REQUEST
        ...
        # Make the POST request to Home Assistant API
        response = requests.post(
            f"{HOME_ASSISTANT_API}/services/select/select_option",
            headers=headers,
            json=payload,
            verify=VERIFY_SSL,
            timeout=10,
        ...
        if response.status_code == HTTPStatus.OK:
            return jsonify(
                {"message": f"Select entity {entity_id} updated successfully."}
            ), HTTPStatus.OK
        return jsonify(
            {
                "error": f"Failed to update select entity {entity_id}.",
                "details": response.text,
            }
        ), response.status_code
        ...
        return jsonify(
            {
                "error": "An error occurred while updating the select entity.",
                "details": str(e),
            }
        ), HTTPStatus.INTERNAL_SERVER_ERROR
...
def light_turn_on() -> tuple[dict[str, Any], int]:
    """Turn on a light entity via Home Assistant API."""
    try:
        data = request.json
        entity_id = data.get("entity_id")
        ...
        # Make the POST request to Home Assistant API
        response = requests.post(
            f"{HOME_ASSISTANT_API}/services/light/turn_on",
            headers=headers,
            json=payload,
            verify=VERIFY_SSL,
            timeout=10,
        ...
        if response.status_code == HTTPStatus.OK:
            return jsonify(
                {"message": f"Light {entity_id} turned on successfully."}
            ), HTTPStatus.OK
        return jsonify(
            {
                "error": f"Failed to turn on light {entity_id}.",
                "details": response.text,
            }
        ), response.status_code
        ...
        return jsonify(
            {
                "error": "An error occurred while turning on the light.",
                "details": str(e),
            }
        ), HTTPStatus.INTERNAL_SERVER_ERROR
...
def light_turn_off() -> tuple[dict[str, Any], int]:
    """Turn off a light entity via Home Assistant API."""
    try:
        data = request.json
        entity_id = data.get("entity_id")
        ...
        # Make the POST request to Home Assistant API
        response = requests.post(
            f"{HOME_ASSISTANT_API}/services/light/turn_off",
            headers=headers,
            json=payload,
            verify=VERIFY_SSL,
            timeout=10,
        ...
        if response.status_code == HTTPStatus.OK:
            return jsonify(
                {"message": f"Light {entity_id} turned off successfully."}
            ), HTTPStatus.OK
        return jsonify(
            {
                "error": f"Failed to turn off light {entity_id}.",
                "details": response.text,
            }
        ), response.status_code
        ...
        return jsonify(
            {
                "error": "An error occurred while turning off the light.",
                "details": str(e),
            }
        ), HTTPStatus.INTERNAL_SERVER_ERROR
...
def set_selected_entities():
    ...
    try:
        ...
        log.exception("Error setting selected entities")
...
def get_selected_entities():
    ...
    global selected_entity_ids
    return jsonify(
        {"entity_ids": list(selected_entity_ids), "count": len(selected_entity_ids)}
    )


def is_mmwave_entity(entity_id: str) -> bool:
...
@sock.route("/ws")
def websocket_proxy(ws: WebSocketApp):
    ...
    def ha_on_open(ha_ws_instance: WebSocketApp) -> None:
        if supervisor_token:
            auth_message = {"type": "auth", "access_token": supervisor_token}
            ha_ws_instance.send(json.dumps(auth_message))
        elif ha_url and ha_token:
            auth_message = {"type": "auth", "access_token": ha_token}
            ha_ws_instance.send(json.dumps(auth_message))
        else:
            log.error("No supervisor token available for WebSocket auth")

    def ha_on_message(ha_ws_instance: WebSocketApp, message: str) -> None:  # noqa: ARG001
        ...
        try:
            ...
            if data.get("type") == "auth_required" or data.get("type") == "auth_ok":
                return
            if data.get("type") == "auth_invalid":
                log.error("HA WebSocket authentication failed")
                return
            ...
            if data.get("type") in ["result"]:
                ...
                if not data.get("success") and "id" in data:
                    ...
                    log.error(
                        "HA subscription failed for ID %s: %s",
                        data.get("id"),
                        error_info,
                    )
                from_ha_queue.put(message)
            ...
            log.exception("Error filtering HA message")
            from_ha_queue.put(message)

    def ha_on_error(
        ha_ws_instance: WebSocketApp,  # noqa: ARG001
        error: str,
    ) -> None:
        log.error("HA WebSocket error: %s", error)
    ...
    def ha_on_close(
        ...
        if close_status_code and close_status_code != 1000:
            log.error(
                "HA WebSocket closed unexpectedly: %s - %s",
                close_status_code,
                close_msg,
            )
    ...
    if supervisor_token:
        ...
        log.error("No supervisor token or HA_URL available for websocket proxy")
        ...
        while proxy_active and ha_thread.is_alive():
            try:
                ...
                if ha_ws and ha_ws.sock and ha_ws.sock.connected:
                    ...
                    log.error("Cannot send to HA - WebSocket not connected")

            except queue.Empty:
                continue
            except Exception:
                log.exception("Error forwarding to HA")
                break
    ...
    try:
        ...
        while proxy_active:
            ...
            try:
                ...
                log.exception("Error sending to frontend")
                break
            ...
            try:
                ...
                if "timeout" not in str(e).lower():
                    ...
                    if "Connection closed: 1000" not in str(e):
                        log.exception("Error receiving from frontend")
                    break

    except Exception:
        log.exception("WebSocket proxy error")
